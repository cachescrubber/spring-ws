...
public abstract class AbstractMarshallingMethodInvocationAdapter implements MethodInvocationAdapter {
	 ...
	 * @return the resolved request payload method parameter, null if no suitable parameter could be resolved.
	...
	/**
	 * Extract the request payload argument value, performing JAXB specific argument processing if necessary.
	 * <p>
	 * If the given method parameter type is annotated with {@link XmlType}, the value at the parameter index is
	 * wrapped inside a {@link JAXBElement}. Otherwise it is returned as-is. The {@link QName} of the JAXBElement
	 * is resolved using the metadata available via the method parameter.
	 *
